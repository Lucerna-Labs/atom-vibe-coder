...
try {
    ...
    if ($AllowProviderBlock) {
        ...
        powershell -ExecutionPolicy Bypass -File (Join-Path $PSScriptRoot "Test-NativeVibeRuntime.ps1")
        if ($LASTEXITCODE -ne 0) { throw "native Vibe runtime gate failed with exit code $LASTEXITCODE" }
