...
function Focus-NativeWindow() {
    ...
    [MathAtomsNativeInputEditing]::ShowWindow($handle, 5) | Out-Null
    [MathAtomsNativeInputEditing]::SetForegroundWindow($handle) | Out-Null
    Start-Sleep -Milliseconds 350
}

function Send-WmChar([int]$Code, [string]$Stage) {
    $handle = Get-AtomNativeWindowHandle -Process $script:proc
    [MathAtomsNativeInputEditing]::PostMessage($handle, 0x0102, [UIntPtr]::new($Code), [IntPtr]::Zero) | Out-Null
...
function Send-KeyDown([int]$Code, [string]$Stage) {
    $handle = Get-AtomNativeWindowHandle -Process $script:proc
    [MathAtomsNativeInputEditing]::PostMessage($handle, 0x0100, [UIntPtr]::new($Code), [IntPtr]::Zero) | Out-Null
    Start-Sleep -Milliseconds 120
    $script:proc = Refresh-NativeProcess $Stage
    if (-not $script:proc.Responding) {
        throw "Native app stopped responding during $Stage"
...
function Clear-FocusedInput() {
    for ($i = 0; $i -lt 180; $i++) {
        $handle = Get-AtomNativeWindowHandle -Process $script:proc
        [MathAtomsNativeInputEditing]::PostMessage($handle, 0x0102, [UIntPtr]::new(8), [IntPtr]::Zero) | Out-Null
...
try {
    ...
    Write-Host "native input editing ok: copied='$copied' title=$(Get-AtomNativeWindowTitle -Process $proc)"
}
finally {
    if (-not $LeaveRunning) {
