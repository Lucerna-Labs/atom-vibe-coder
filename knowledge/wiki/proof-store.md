...
Persistent store writes are fail-closed. A proof may only be learned back into the wiki graph after the JSONL append succeeds; a write failure emits StoreBlocked on the Spiderweb Bus and prevents the UI from claiming a proven run.
